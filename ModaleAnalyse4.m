function [m,c,w,phi]= ModaleAnalyse4(M,C,x0,t)
...
phi1 = Phi(:,1:length(M)) ;
phi  = [phi1(:,1)/phi1(1,1)   phi1(:,2)/phi1(1,2)  phi1(:,3)/phi1(1,3) phi1(:,4)/phi1(1,4)];
...
y4 = y0(4,1)*cos(w(4,4)*t);
subplot(2,1,1), plot(t,y1, t,y2, t,y3, t,y4,'LineWidth',2)
set(gca,'FontSize',16)
...
x1 = phi(1,1)*y1 + phi(1,2)*y2 + phi(1,3)*y3 + phi(1,4)*y4;
x2 = phi(2,1)*y1 + phi(2,2)*y2 + phi(2,3)*y3 + phi(2,4)*y4;
x3 = phi(3,1)*y1 + phi(3,2)*y2 + phi(3,3)*y3 + phi(3,4)*y4;
x4 = phi(4,1)*y1 + phi(4,2)*y2 + phi(4,3)*y3 + phi(4,4)*y4;
subplot(2,1,2), plot(t,x1, t,x2, t,x3, t,x4,'LineWidth',2)
set(gca,'FontSize',16)
