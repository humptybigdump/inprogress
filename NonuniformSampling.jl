...
# ╔═╡ f3b9f2a1-c69c-4e02-9f0f-99828cf2acbd
begin
	using PlutoUI
	using WGLMakie
	using Distributions
...
# ╔═╡ 2208ba34-2486-4962-8869-c8260af609f2
md"""
## Sampling Methods for Machine Learning (SML)
"""

# ╔═╡ 6ce90e6b-7c4c-4743-a459-dfe9c569dbbd
md"""
#### Packages used
"""

# ╔═╡ d2a573cb-f5a7-49ca-b5b4-ceb5cde189b7
md"""
### Mappings Between Random Variables
"""

# ╔═╡ 2bd9abc5-219b-4a0e-980b-7468f2b5f627
md"""
#### Leaky rectified linear unit (ReLU)
"""

# ╔═╡ 300968f3-19fa-4e2e-bf53-8b0058227c4f
md"""
#### Knothe-Rosenblatt rearrangement
"""

# ╔═╡ f7c34a79-2a56-4ec9-8226-ac7ae8c87ee6
md"""
#### Quantile function of univariate Gaussian
"""

# ╔═╡ feed115c-edb9-48b5-8bc4-9134beb58026
	function Q(u, μ, σ)
		μ + sqrt(2) * σ * erfinv(2 * u - 1) 
	end

# ╔═╡ 0314560a-ae9e-4a65-b82b-59f482ff0b72
let
	function KR(μ, C, numSamples)
		...
		for i ∈ 1:numSamples
			...
			u1 = rand()
			y1 = Q(u1, m1, sqrt(C1))
			m2 = μ[2] + C[1,2] / C[1,1] * (y1 - μ[2])
			C2 = C[2,2] - C[1,2] * C[2,1] / C[1,1]
			u2 = rand()
			y2 = Q(u2, m2, sqrt(C2))
			Y[:, i] = [y1;y2]
		end
	...
	fig = Figure()
	numPlots = 2
	ax = [Axis(fig[i, 1]) for i ∈ 1:numPlots]
	for i ∈ 1:numPlots
		ax[i].aspect = 1
		ax[i].limits = (-4, 4, -4, 4)
	end
	scatter!(ax[1], X[1, :], X[2, :], markersize=2, color=:red)
	scatter!(ax[2], Y[1, :], Y[2, :], markersize=2, color=:green)
	fig
end

# ╔═╡ 6d7b8a05-3272-4f36-afb9-81771df07569
md"""
##### Density of Gaussian random variable
"""

# ╔═╡ 1169989a-0d23-4cf6-8158-b86d32f5cf94
function N(x, μ, σ)
	exp(-0.5 * (x - μ)^2 / σ^2) / (sqrt(2 * pi) * σ)
end

# ╔═╡ 02b3acaa-0b16-4536-a80b-617748997e74
let
	a = 0.2
	b = 1.0
	ReLU(x) = x < 0 ? a * x : b * x 
	...
	x = σ * randn(L) .+ μ
	y = ReLU.(x)
	f(y) = y < 0 ? N(y / a, μ, σ) / a : N(y / b, μ, σ) / b	
	fig = Figure()
	numPlots = 2
	ax = [Axis(fig[i, 1]) for i ∈ 1:numPlots]
	hist!(ax[1], y, bins = 1000, normalization = :pdf)
	z = LinRange(-3, 3, 1000)
	lines!(ax[2], z, f.(z))
	for i ∈ 1:numPlots
		ax[i].limits = (-1, 2, 0, 2.5)
	end
	fig
end

# ╔═╡ 1221c313-6106-4810-aac7-118510c03302
md"""
#### Learning with KLD and Finite Differences
- Zoom: `scroll`
- Zoom single axis: `scroll` holding `X` or `Z`
- Drag pan: `rightclick`
- Restore: `Strg + leftclick`
"""

# ╔═╡ 2f8ccb8b-6285-4234-a94f-2e2a02cbb1a0
let
	# y Grid
	h = 0.1
	y = collect(-6.0 : h : 6.0)
	# Density fx 
	Cx = 1/2
	fx(x) = exp(-1/2 * x^2 / Cx) / sqrt(2*pi*Cx)
	# Density fy: GAUSS
	m = 1
	Cy = 2
	# fy(y) = exp(-1/2 * (y - m)^2 / Cy) / sqrt(2*pi*Cy)
	# fy(y) = exp(-1/2 * (y - m)^2 / Cy) / sqrt(2*pi*Cy) * (1+sin(2y))
	# Density fy: FANCY, with numerically computed normalization constant 
	poly = integrate(-fromroots([-3,-2,-1,2,4])) * .01
	fyu(y) = exp( poly(y) ) # unnormalized
	fyu(3) # test run
	probI = IntegralProblem((u,p)->fyu(u), (-Inf, Inf))
	solI = solve(probI, HCubatureJL())
	@assert(isfinite(solI.u), "Use uneven number of roots in poly, otherwise no normalizable density!")
	fy_norm = solI.u
	fy(y) = fyu(y) / fy_norm
	# Positivity Mapping (alternative to constraint)
	function pos(x)
	    if x>0
	        z = x + log(1 + exp(-x))
	    else
	...
	# Read optimization parameter vector
	function convert(η)
		local N, Δ, r
	    N = length(η)-1  # number of samples
	    Δ = pos.(η[2:end])  # slopes 
	    r = Vector{eltype(η)}(undef, N)   # reverse mapping 
	    # cumulative sum
	    r[1] = η[1]
	    for k ∈ 2:N
	        r[k] = r[k-1] + (Δ[k-1] + Δ[k])/2
	    end
	    return r, Δ, N
	end
	# Objective Function
	d(k, r, Δ, y) = - h * fy(y[k]) * (log(Δ[k]/h) - r[k]^2)  # KLD, for Cx=1/2 
	function objFct(η, y)
		local N, Δ, r
	    r, Δ, N = convert(η)
	    D = 0
	...
	# Initial Guess
	N = length(y)
	η₀ = vcat(-2.0, randn(N) .- 3)
	objFct(η₀, y) # test run
	# Optimize
	objAD = OptimizationFunction(objFct, AutoForwardDiff()) 
	optProb = OptimizationProblem(objAD, η₀, y)
	sol = solve(optProb, BFGS(), maxiters = 50)  
	η = sol.u
	r, Δ, N = convert(η)
	r₀, Δ₀, N₀ = convert(η₀)
	## Plot
	...
	fig = Figure()
	ax = [Axis(fig[1,1]), Axis(fig[1,2]), Axis(fig[2,2])]
	yrange = [minimum(y), maximum(y)]
	xrange = [minimum(r), maximum(r)]
	colgap!(fig.layout, 0)
	rowgap!(fig.layout, 0)
	# r
	hr0 = lines!(ax[2], y, r₀, color = :grey)
	lines!(ax[2], y, r)
	hr = scatter!(ax[2], y, r, markersize = 8, strokecolor = :black, strokewidth = .5)
	limits!(ax[2], yrange, xrange)
	hidedecorations!(ax[2], grid = false, label = false)
	ax[2].xaxisposition = :top
	ax[2].yaxisposition = :right
	ax[2].xlabel = L"y"
	ax[2].ylabel = L"r\,(y)"	
	# fy 
	fz = fx.(r[1:N]) .* Δ/h
	hfy = lines!(ax[3], y[1:N], fz[1:end], color=:red)
	hfygt = lines!(ax[3], y[1:N], fy.(y[1:N]), color=:green, linestyle=:dash)
	xlims!(ax[3], yrange)
	ax[3].yaxisposition = :right
	ax[3].xlabel = L"y"
	# fx
	hfx = lines!(ax[1], fx.(r), r)
	ax[1].xreversed = true
	ylims!(ax[1], xrange)
	...
	linkxaxes!(ax[2], ax[3])
	linkyaxes!(ax[1], ax[2])
	Legend(fig[2,1], [hr0, hr, hfx, hfy, hfygt], ["initial guess", L"\text{learned } r(y)", L"f_x(x)", L"f_y(y)", L"\tilde{f}_y(y)"], "Trafo Learning", tellheight = false, tellwidth = false )
	fig
end

# ╔═╡ 007ef61d-5526-4ec2-a148-bfdccca7bc07
md"""
#### Learning with ISD and Finite Differences
"""

# ╔═╡ c9974cd5-2af2-46c0-ad85-16fa0c6aa283
let
	# y Grid
	h = 0.1
	y = collect(-6.0 : h : 6.0)
	...
	# Objective Function
	d(k, r, Δ, y) = h * (fy(y[k]) - fx(r[k]) * Δ[k]/h)^2  # ISD
	function objFct(η, y)
		local N, Δ, r
	    r, Δ, N = convert(η)
	    D = 0
...
manifest_format = "2.0"
project_hash = "857bee03ec46e726dbb2955b801c3af28874a052"
