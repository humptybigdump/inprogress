...
struct TextBackupState {
    ...
    void backup() {
        ...
        /*        attribs = (vtxAttribData*)malloc(sizeof(vtxAttribData)*maxVtxAttribs*2);
...
inline GLuint OpenGLText::LinkGLSLProgram(GLuint vertexShader, GLuint fragmentShader) {
    ...
    char* infoLog = new char[infoLogLength + 16384];
...
bool OpenGLText::init(unsigned char* imageData, FileHeader* glyphInfos_, int w, int h) {
    ...
    glTextureImage2DEXT(m_fontTex, GL_TEXTURE_2D, 0, extFmt, glyphInfos->texwidth, glyphInfos->texheight, 0, extFmt,
...
bool OpenGLText::init(const char* fontName, int w, int h) {
    ...
    if (fontName) {
        char fname[1024];
        std::snprintf(fname, 1024, "%s.tga", fontName);
        TGA* fontTGA = new TGA;
        TGA::TGAError err = fontTGA->load(fname);
        if (err != TGA::TGA_NO_ERROR)
            return false;
        std::snprintf(fname, 1024, "%s.bin", fontName);
        ...
        if (fd) {
            ...
            switch (fontTGA->m_texFormat) {
                ...
                case TGA::ALPHA:
                    ...
                    break;
                case TGA::UNKNOWN:
                    printf("[FONT]: unknown TGA format.\n");
                    exit(1);
            }
            glTextureImage2DEXT(m_fontTex, GL_TEXTURE_2D, 0, extFmt, u, h, 0, extFmt, GL_UNSIGNED_BYTE,
                                fontTGA->m_nImageData);
            glTextureParameterf(m_fontTex, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            glTextureParameterf(m_fontTex, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            glTextureParameterf(m_fontTex, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            glTextureParameterf(m_fontTex, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
bool OpenGLText::init(int w, int h) {
    ...
    if (m_widgetProgram == 0) {
        ...
        float* tcs = new float[256 * 4 + 16384];
        for (int i = 0; i < 256; i++) {
...
void OpenGLText::endString() {
    ...
    if (nVertices > 0) {
        ...
        //		if ( m_vbosz < nVertices )
        {
            ...
            //            glBufferData(GL_TEXTURE_BUFFER, nVertices*sizeof(Vertex), &(m_vertices[0]), GL_STREAM_DRAW);
            glBufferData(GL_TEXTURE_BUFFER, nVertices * sizeof(Vertex), m_vertices, GL_STREAM_DRAW);
        ...
        err = glGetError();
        m_color = glGetUniformLocation(m_widgetProgram, "color");
        glProgramUniform4fv(m_widgetProgram, m_color, 1, mycolorf);
        err = glGetError();
        err = err;
        //
        // Draw
...
float OpenGLText::drawString(int x, int y, const char* text, int nbLines, float* color4f) {
    ...
    memcpy(mycolorf, color4f, sizeof(float) * 4);
