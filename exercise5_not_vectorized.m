...
mysum = 1 + c_ini(1)/K(1) + c_ini(2)/K(2);
s_ini(1) = s_max*c_ini(1)/K(1)/mysum;
s_ini(2) = s_max*c_ini(2)/K(2)/mysum;
...
mysum = 1 + c(1)/K(1) + c(2)/K(2);
s(1)  = s_max*c(1)/K(1)/mysum;
s(2)  = s_max*c(2)/K(2)/mysum;
...
while max(abs(m_error)./m_new) > tol
    % increase counter
    iter=iter+1;
    % update aqueus-phase concentrations
    c(1)=m_new(1)/(Vw+m_s*s_max/K(1)/mysum);
    c(2)=m_new(2)/(Vw+m_s*s_max/K(2)/mysum);
