...
### M3: hQ-Beziehung und Durchflusszeitreihe ###
...
df = pd.read_csv('S3_hQ.csv', sep = '\t', comment = '#', decimal = ',') # Separator ; Kommazeichen .
...
hs_lang = pd.read_csv('S3_Druck.csv', sep = ';', comment = '#')
hs_lang['datetime'] = pd.to_datetime(hs_lang['Datum/Zeit'])
hs_lang = hs_lang.drop(columns = ['Kurzname'])
...
hs['Wasserstand [mWS]'].loc[hs['datetime'] == '2025-08-06 16:00:00'] = np.nan

#%% Wasserstandsinformationen für gewünschten Zeitpunkt abrufen
info = hs.loc[hs['datetime'] == '2025-08-06 16:05:00']
print(info)
print('\n')
...
def Q_kali_log(c1, c2, h):
    return c1*(h**c2)
h_reg = np.linspace(0.23, 0.38, 50).reshape(-1, 1)  # Modelleriungsbereich erstellen
...
ax[0].set_title('h-Q-Beziehung M3', fontsize = 25, pad = 12)
...
ax[1].set_title('Linearisierte h-Q-Beziehung M3', fontsize = 25, pad = 12)
...
#ax.set_title('Durchflusszeitreihe M3', fontsize = 25, pad = 12)
...
hs.to_csv('M3_Durchflusszeitreihe.csv', index = False)  # 'index = False' entfernt die Indexspalte
...
#ax.set_title('Unsicherheit Durchfluss M3 Tracerversuch', fontsize = 25, pad = 12)
ax.set_xlabel('Zeit', fontsize = 18)
ax.set_ylabel('Durchfluss Q [$\mathrm{m^3\ s^{-1}}$]', fontsize = 18)
ax.grid()
