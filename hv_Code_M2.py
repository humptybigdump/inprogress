...
### M2: hv-Beziehung und Geschwindigkeitszeitreihe ###
...
df = pd.read_csv('S2_hv.csv', sep = '\t', decimal = ',', comment = '#')
...
pw = pd.read_csv('S2_Druck.csv', sep = '\t', decimal = ',', comment = '#')
...
hs['Wasserstand [mWs]'].loc[hs.index == '2025-08-04 14:20:00'] = np.nan
hs['Wasserstand [mWs]'].loc[hs.index == '2025-08-05 14:40:00'] = np.nan
hs['Wasserstand [mWs]'].loc[hs.index == '2025-08-05 14:50:00'] = np.nan
hs['Wasserstand [mWs]'].loc[hs.index == '2025-08-08 17:00:00'] = np.nan

#%% Wasserstandsinformationen für gewünschten Zeitpunkt abrufen
info = hs.loc[hs.index == '2025-08-08 09:00:00']
print(info)
print('\n')
...
def v_kali_log(c1, c2, h):
    return c1*(h**c2)
h_reg = np.linspace(0.03, 0.18, 50).reshape(-1, 1)  # Modelleriungsbereich erstellen
...
ax[0].set_title('h-v-Beziehung M2', fontsize = 25, pad = 12)
...
ax[1].set_title('Linearisierte h-v-Beziehung M2', fontsize = 25, pad = 12)
...
ax.set_title('Geschwindigkeitszeitreihe M2', fontsize = 25, pad = 12)
ax.set_xlabel('Zeit', fontsize = 18)
ax.set_ylabel('Geschwindigkeit v [$\mathrm{m\ s^{-1}}$]', fontsize = 18)
...
hs.to_csv('M2_Geschwindigkeitszeitreihe.csv', index = True)   # Index wird mit exportiert
