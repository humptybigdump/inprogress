 ...
 * 
 * this is version 2.10; this defaults to a canonical
 * algorithm, obtained by periodically refreshing the demons; it is
 * not quite Metropolis since I don't refresh very often for speed
 * reasons, and the upper limit on the demon energy rejects a few
 * changes that the Metropolis algorithm would make.  The algorithm
 * is, however, "exact."

  the only change from version 2.9 is to work around a bug
  in mrand49() in glibc where all upper 16 bits are equal
...
/* this new version (July `97) is supposed to be more readable */
{
  ...
  for (rowloop = 0; rowloop < 4 * nrows; rowloop++) {
	...
	newstuff = currentword | cluster[nrow][col] | cluster[row][ncol];
	if (newstuff) {
	  newstuff = 0;
	  ...
	  if (1 & (sadx[row][col])) {
	    if (1 & (currentword ^
	      ((cluster[row][ncol = nextcol]) >> (WORDSIZE - 1)))) {
	      newstuff = 1;
